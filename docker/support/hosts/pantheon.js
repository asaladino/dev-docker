...
module.exports = {
  ...
  isWordpress: function () {
    let sourcePath = '.env';
    let ee = envfile.parseFileSync(sourcePath);
    return fs.existsSync('./' + ee['WEB_APP_DIR'] + '/wp-admin');
  },
  /**
  ...
  copySettings: function () {
    ...
    if (this.isWordpress()) {
      ...
      let targetFile = './' + ee['WEB_APP_DIR'] + '/wp-config-local.php';
      fs.writeFileSync(targetFile, fs.readFileSync(sourceFile));
    }
    else {
      let sourceFile = './docker/apache/drupal/settings.local.php';
      let targetFile = './' + ee['WEB_APP_DIR'] + '/sites/default/settings.local.php';

      let settingsFile = './' + ee['WEB_APP_DIR'] + '/sites/default/settings.php';
